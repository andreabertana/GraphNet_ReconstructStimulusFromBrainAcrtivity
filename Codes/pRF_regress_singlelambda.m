...
[est_prfs] = fitGaussian_realData(B, SH, X2, Y, nrVox, HIRF, true_parameters);
...
lambda_range = [.1, .5, 1 , 5, 7, 10,  13, 16, 20, 25, 30];
alpha_range = 0;
...
for i = 1:Ngridsearch
    ...
    fprintf('\nGrid search iteration %2d/%2d, Lambda: %3.2g, ', i, Ngridsearch, lambda);
...
%     R2(i, :) = 1 - sum(bsxfun(@minus, res, mean(res)).^2) ./ sum(bsxfun(@minus, B_noint, mean(B_noint)).^2);    
    ...
    R2allv(i,:) = 1 - sum(res.^2)./sum(B_noint.^2);
    R2(i) = 1 - sum(res(:).^2)/sum(B_noint(:).^2);
    fprintf('R2: %3.2f\n', R2(i));
...
%     vox_ind = best_lambda==unique_bl(j);
    EstimatedpRFs = (X_allruns'*X_allruns + best_lambda*Iplus)\X_allruns'*B;            
% end

% ests = nan(Ncoeff, length(lambda_range), nrVox);
% for i = 1: nrVox
%     data = B(:,i);
%     for j =1:length(lambda_range)
%         lambda = lambda_range(j);
%         ests(:, j) =(X_allruns'*X_allruns + lambda*Iplus)\X_allruns'*data;
%         pred = X_allruns * ests(:, j);
%         pred_int = X_allruns(:, end-nrRuns+1:end)*ests(end-nrRuns+1:end, j);
%         res = data - pred;
%         Bnoint = data - pred_int;
%         R2_vox1(1,j,i) = 1 - sum(res.^2)./ sum(Bnoint(:,1).^2);
%     end
% end
...
PredictedTs_int = X_allruns(:, end-nrRuns+1:end)*EstimatedpRFs(end-nrRuns+1:end, :);

res_pv = B - PredictedTs;
B_noint = B - PredictedTs_int;
R2_pv = 1- sum(res_pv.^2)./sum(B_noint.^2);
...
if nargin ~= 0 
    save([path,filesep, subj_id, '_pRFestimates.mat'],'PredictedTs_int', 'imdim', 'R2_pv' , 'degree', 'noiseVar', 'EstimatedpRFs', 'best_lambda', 'bestR2', 'nrRuns', 'Intercept', 'Pix', 'used_lambda', 'PredictedTs')
end
%%
